'''
TITLE:
Range Extraction

DESCRIPTION:
A format for expressing an ordered list of integers is to use a comma separated list of
either

individual integers
or a range of integers denoted by the starting integer separated from the end integer in
the range by a dash, '-'. The range includes all integers in the interval including both
endpoints. It is not considered a range unless it spans at least 3 numbers. For example
"12,13,15-17"
Complete the solution so that it takes a list of integers in increasing order and returns
a correctly formatted string in the range format.

Example:

solution([-10, -9, -8, -6, -3, -2, -1, 0, 1, 3, 4, 5, 7, 8, 9, 10, 11, 14, 15, 17, 18,
19, 20])
# returns "-10--8,-6,-3-1,3-5,7-11,14,15,17-20"
Courtesy of rosettacode.org

KYU & CREDITS:
4 kyu jhoffner
https://www.codewars.com/kata/51ba717bb08c1cd60f00002f
'''


def solution(n):
    temp = [str(n[0])]
    last_n = n[0]
    final = []

    i = 1
    while i < len(n):
        # if the number is a continuation of the last number: add it to the 'temp' list
        if n[i] == last_n + 1 or len(temp) == 0:
            temp.append(str(n[i]))
            last_n = n[i]

        # else: add the 'temp' list to the 'final' list
        else:
            add_temp_to_final(final, temp)
            last_n = n[i]
            temp = []
            i -= 1
        i += 1

    add_temp_to_final(final, temp)
    return ','.join(final)


def add_temp_to_final(final, temp):
    if len(temp) > 2:
        final.append(f'{temp[0]}-{temp[len(temp) - 1]}')
    else:
        final.extend(temp)


print(solution([-6, -3, -2, -1, 0, 1, 3, 4, 5, 7, 8, 9, 10, 11, 14, 15, 17, 18, 19, 20]))



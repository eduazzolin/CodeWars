...
def good_vs_evil(good, evil):
    good_count = map(int, good.split())
    evil_count = map(int, evil.split())
    good_worth = [1, 2, 3, 3, 4, 10]
    evil_worth = [1, 2, 2, 2, 3, 5, 10]
    good_total = sum([c * w for c, w in zip(good_count, good_worth)])
    evil_total = sum([c * w for c, w in zip(evil_count, evil_worth)])

    if good_total > evil_total: return 'Battle Result: Good triumphs over Evil'
    if good_total < evil_total: return 'Battle Result: Evil eradicates all trace of Good'
    if good_total == evil_total: return 'Battle Result: No victor on this battle field'
